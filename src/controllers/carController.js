...
carController.addToCart = async (req, res) => {
  ...
  try {
    ...
    if (!product || !user) { 
      return res.json({ add: false, error: "Usuario y/o Producto no encontrado", product,username});
    }
    ...
    if (existingProduct) {
      ...
      user.shopping_car.total += parseFloat((existingProduct.price * cantidad).toFixed(2));
      ...
      user.shopping_car.total += parseFloat((newCartItem.price * cantidad).toFixed(2));
      user.shopping_car.cantidad += cantidad;
